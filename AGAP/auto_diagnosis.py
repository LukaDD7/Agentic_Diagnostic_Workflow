...
import os
import sys
sys.path.append('/media/luzhenyang/project/agent_graph_diag')
import csv
import json
import time
import pandas as pd
...
from extract_patient_info import Template_customized, MedicalDataset
...
class NodesAndEdeges:
    def __init__(self, nodes, edges, prompt_mode='lenient'):
        ...
        self.nodes_mapping = {
            ...
            # 'str_ab_hpi': self.str_ab_hpi, 
            # 'str_ab_pe': self.str_ab_pe,
            # 'str_ab_lab': self.str_ab_lab,
            # 'str_ab_ima': self.str_ab_ima,
            ...
            # 'str_diag_app': self.str_diag_app,
            # 'str_diag_cho': self.str_diag_cho,
            # 'str_diag_pan': self.str_diag_pan,
            # 'str_diag_div': self.str_diag_div,
...
class AgentGraph:
    ...
    def run_full_dataset(self):
        ...
        # output process
        output_file_path = f'/media/luzhenyang/project/agent_graph_diag/results/AGAP_full_dataset_{self.prompt_mode}.csv'
        fieldnames = [
            'hadm_id', 'diagnosis', 
            'ab_hpi', 'ab_pe', 'ab_lab', 'ab_ima', 
            'diag_app', 'diag_cho', 'diag_div', 'diag_pan', 
            ...
            with open(output_file_path, 'a', newline='', encoding='utf-8') as csvfile:
                ...
                if True:    # called tool
                    ...
                    for ab_ in ['ab_hpi', 'ab_pe', 'ab_lab', 'ab_ima']:
                        msg = result[ab_]
                        input_tokens_acc += msg.usage_metadata[0]['input_tokens']
                        output_tokens_acc += msg.usage_metadata[0]['output_tokens']
                        total_tokens_acc += msg.usage_metadata[0]['total_tokens']
                    ...
                    saved_indices_set = set(saved_indices)

                    other_process = [
                        msg for idx, msg in enumerate(all_msgs)
                        if idx - len(all_msgs) not in saved_indices_set
                    ]
